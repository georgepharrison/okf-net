...
public class McpProtocolTests
{
    ...
    public void InitializeAnswersWithTheNegotiatedRevisionTheToolCapabilityAndTheDoctrine()
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, McpHarness.Initialize());
        ...
        Assert.Equal(McpHarness.ProtocolVersion, result.GetProperty("protocolVersion").GetString());
    ...
    public void AnOlderSupportedRevisionIsEchoedBackAndAnUnknownOneIsAnsweredWithTheLatest()
    {
        ...
        var run = McpHarness.Session(
            Environment(vault),
            vault.Root,
            McpHarness.Initialize(1, "2024-11-05"),
            McpHarness.Initialize(2, "1999-01-01"));
        ...
        Assert.Equal(McpHarness.ProtocolVersion, unknown.RootElement.GetProperty("protocolVersion").GetString());
    }
    ...
    public void PingAnswersWithAnEmptyResult()
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, McpHarness.Request(9, "ping"));
    ...
    public void AnUnknownMethodIsMethodNotFound()
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, McpHarness.Request(3, "resources/list"));
    ...
    public void AMessageThatIsNotAJsonRpcRequestIsRejectedWithoutStoppingTheServer(string message)
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, message, McpHarness.Request(6, "ping"));
    ...
    public void AStringThatCannotBeReadBackIsAnsweredRatherThanFatal(string message)
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, message, McpHarness.Request(7, "ping"));
    ...
    public void EveryResponseIsExactlyOneLine()
    {
        ...
        var run = McpHarness.Session(
            ...
            McpHarness.Call(3, "okf_search", """{"query":"widget"}"""),
            McpHarness.Call(4, "okf_read", """{"path":"widgets.md"}"""),
            McpHarness.Call(5, "okf_list"));
    ...
    public void ClosingStdinIsACleanShutdown()
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root);
    ...
    public void ToolsListNamesTheThreeReadOnlyToolsWithSchemasAndTheDoctrine()
    {
        ...
        var run = McpHarness.Session(Environment(vault), vault.Root, McpHarness.Request(2, "tools/list"));
    ...
    public void McpHelpIsSuccessAndDescribesTheStdioContract()
    {
        ...
        var run = McpHarness.Run(Environment(vault), ["--help"]);
    ...
    public void AnUnresolvableVaultIsAStartupFailureRatherThanAServerThatFailsEveryCall()
    {
        ...
        var run = McpHarness.Session(
            CliHarness.Environment(empty.Root, empty.Root),
            null,
            McpHarness.Initialize());
    ...
    public void VerboseReportsResolutionOnStderrAndLeavesStdoutToTheProtocol()
    {
        ...
        var run = McpHarness.Run(Environment(vault), ["--verbose", vault.Root], McpHarness.Initialize());
    ...
    internal static OkfEnvironment Environment(TempTree vault) =>
        CliHarness.Environment(vault.Root, vault.Root);
}
