...
public class McpToolArgumentTests
{
    ...
    public void ListRefusesAPathThatLeavesTheBundle(string path)
    {
        ...
        var run = McpHarness.Session(
            ...
            vault.Root,
            McpHarness.Call(1, "okf_list", $$"""{"path":"{{path}}"}"""));
    ...
    public void ListNormalizesAnUntidyPathToTheBundleRoot(string path)
    {
        ...
        var untidy = McpHarness.Session(environment, vault.Root, McpHarness.Call(1, "okf_list", $$"""
            {"bundle":"searchable","path":"{{path}}"}
            """));
        var plain = McpHarness.Session(environment, vault.Root, McpHarness.Call(1, "okf_list", """
            {"bundle":"searchable"}
            """));
    ...
    public void ListWithoutArgumentsReportsTheScopeWithConceptCounts()
    {
        ...
        var run = McpHarness.Session(
            CliHarness.Environment(tree.Root, tree.Root),
    ...
    public void AnUnknownBundleNameIsRefusedWithTheNamesThatAreInScope()
    {
        ...
        var run = McpHarness.Session(
            McpProtocolTests.Environment(vault),
            vault.Root,
            McpHarness.Call(1, "okf_list", """{"bundle":"nowhere"}"""));
    ...
    public void ADuplicatedBundleNameIsRefusedAsAmbiguousAndLabelledByRoot()
    {
        ...
        var environment = CliHarness.Environment(tree.Root, tree.Root);

        Assert.Equal(
            CliApplication.ExitSuccess,
            CliHarness.Run(environment, "register", Path.Combine(tree.Root, "one")).ExitCode);
        Assert.Equal(
            CliApplication.ExitSuccess,
            CliHarness.Run(environment, "register", Path.Combine(tree.Root, "two")).ExitCode);

        var run = McpHarness.Run(
            environment,
            ["--scope", "registered"],
            McpHarness.Call(1, "okf_list", """{"bundle":"notes"}"""));
    ...
    public void AMissingConceptNamesTheSingleBundleItWasLookedForIn()
    {
        ...
        var run = McpHarness.Session(
            McpProtocolTests.Environment(vault),
            vault.Root,
            McpHarness.Call(1, "okf_read", """{"path":"nowhere.md"}"""));
    ...
    public void AFilterThatIsNeitherAStringNorAnArrayOfStringsIsAProtocolError(string arguments)
    {
        ...
        var run = McpHarness.Session(
            McpProtocolTests.Environment(vault),
            vault.Root,
            McpHarness.Call(1, "okf_search", arguments));
    ...
    public void AScalarFilterMeansTheOneElementArray()
    {
        ...
        var scalar = McpHarness.Session(environment, vault.Root, McpHarness.Call(1, "okf_search", """
            {"query":"widget","type":"concept"}
            """));
        var array = McpHarness.Session(environment, vault.Root, McpHarness.Call(1, "okf_search", """
            {"query":"widget","type":["concept"]}
            """));
