...
public class SearchScopeTests
{
    ...
    public void TheDefaultScopeIsTheProjectVaultEvenWithARegistryFullOfEntries()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--verbose");
    ...
    public void RegisteredScopeSeesTheRegisteredVaultAndNotTheProject()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", "registered");
    ...
    public void PersonalScopeSeesThePersonalVaultWithoutItBeingRegistered()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", "personal");
    ...
    public void AllSpansBothRootsAndDisambiguatesEveryResultByItsRoot()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", "all");
    ...
    public void ConfigurationSetsTheScopeAndTheProjectLayerBeatsTheGlobalOne()
    {
        ...
        var fromGlobal = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--verbose");

        File.WriteAllText(
            Path.Combine(world.Project, "okf", "okf.json"),
            """{ "search": { "scope": "project" } }""");
        var fromProject = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--verbose");
        var fromFlag = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", "registered", "--verbose");
    ...
    public void AnUnknownScopeValueIsAUsageFailure()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", "everything");
    ...
    public void APathAndAScopeTogetherAreRefusedRatherThanOneBeingIgnored()
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "okf", "--scope", "all");
    ...
    {
        using var world = new World();
        var environment = CliHarness.Environment(world.Project, world.Home.Root);

        var cli = CliHarness.Run(environment, "search", Query, "--scope", scope, "--json");
        var server = McpHarness.Run(
            environment,
            ["--scope", scope],
            McpHarness.Initialize(),
            McpHarness.Call(2, "okf_search", $$"""{"query":"{{Query}}"}"""));
    ...
    public void TheServerReportsItsScopeAndTheLayerAtStartup()
    {
        ...
        var run = McpHarness.Run(environment, ["--scope", "all", "--verbose"], McpHarness.Initialize());
    ...
    public void CollidingBundleNamesAreDisambiguatedByLabelRatherThanByANewToolArgument()
    {
        ...
        var environment = CliHarness.Environment(world.Project, world.Home.Root);
        var run = McpHarness.Run(
            environment,
            ["--scope", "all"],
            McpHarness.Initialize(),
            McpHarness.Call(2, "okf_list"),
            McpHarness.Call(3, "okf_list", """{"bundle":"notes"}"""));
    ...
    public void AMalformedRegistryIsTheUsageExitCodeAndTheFileIsLeftExactlyAsItWas(string scope)
    {
        ...
        var run = CliHarness.RunIn(world.Project, world.Home.Root, "search", Query, "--scope", scope);
    ...
    public void TheServerRefusesToStartOnAMalformedRegistryRatherThanCrashing()
    {
        ...
        var run = McpHarness.Run(environment, ["--scope", "registered"], McpHarness.Initialize());
    ...
    private sealed class World : IDisposable
    {
        public World(bool register = true)
        {
            ...
            CliHarness.RunIn(Home.Root, Home.Root, "init", "--personal");
            Concept(Path.Combine(Home.Root, "okf", "bundles", "okf", "personal-only.md"), "personal-only");

            if (register)
            {
                CliHarness.RunIn(Registered, Home.Root, "register");
            }
        }
        ...
        {
            CliHarness.RunIn(project, project, "init", "--name", "notes");
