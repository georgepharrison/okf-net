...
public class SiteCommandTests
{
    ...
    public void HelpExitsZeroAndNamesTheRequiredOutputDirectory()
    {
        ...
        var run = CliHarness.RunIn(home.Root, home.Root, "site", "--help");
    ...
    public void AMissingOutputDirectoryIsAUsageFailure()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", bundle);
    ...
    {
        using var tree = new TempTree();
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", "--out", "site", "--nope");
    ...
    public void GeneratingABundleWritesALandingPageAndAPagePerConcept()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", output, "--name", "Fixture");
    ...
    public void GeneratingIntoTheBundleItRendersIsRefused()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", Path.Combine(bundle, "site"));
    ...
    public void GeneratingIntoTheDirectoryHoldingTheBundleIsRefused()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", tree.Root, "--out", bundles);
    ...
    public void SingleFileWritesExactlyOneFile()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", output, "--single-file");
    ...
    public void TheJsonReportCarriesTheTileCounts()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", output, "--json");
    ...
    public void TwoRunsOverAnUnchangedVaultProduceTheSameBytes()
    {
        ...
        Assert.Equal(CliApplication.ExitSuccess, CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", first).ExitCode);
        Assert.Equal(CliApplication.ExitSuccess, CliHarness.RunIn(tree.Root, tree.Root, "site", bundle, "--out", second).ExitCode);
    ...
    public void TheDogfoodVaultGeneratesWithoutError()
    {
        ...
        var run = CliHarness.RunIn(tree.Root, tree.Root, "site", vault, "--out", output);
