...
record a machine-confirmed verification — `verified: [{ by: process:<id>, at:
<now> }]` — when it re-checked the claim against its source; there is no command
for that yet, so it is written by hand and only by whoever did the checking. A
human clears a concept with `okf verify`, which stamps `human:<id>`. A bare
`{ by, at }` mapping is a one-element list: normalize it to a list and append.
