...
| C# style as the code stands | File-scoped namespaces everywhere (146 of 146 files under `src/` and `tests/`). Private instance fields are plain `camelCase`, disambiguated with `this.` — no underscore prefix appears anywhere in the tree. `var` is used freely (~3700 sites). `Nullable`, `TreatWarningsAsErrors`, `AnalysisLevel=latest-all`, `EnforceCodeStyleInBuild` and `CodeAnalysisTreatWarningsAsErrors` are set once in `Directory.Build.props` for all four projects, and a repo-root `.editorconfig` holds the style, naming and every analyzer downgrade, each with its reason ([#31](https://gitlab.tychostation.dev/ringo/okf-net/-/issues/31), `docs/spikes/2026-08-16-analyzers.md`). Two of Ringo's own preferences are **not** yet configured, deliberately, because both are tree-wide rewrites awaiting his call: private fields as `_camelCase`, and explicit types plus target-typed `new` in place of `var`. Until then this row records what is true, not what is wanted. |
