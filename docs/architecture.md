...
- **Rule:** Knowledge lives at `<project>/okf/`, holding `README.md`, `okf.json`,
  ...
  touching a byte outside the fence, and left as found when the file already carries more
  than one fence) and a one-line `CLAUDE.md` naming it (written only when none exists). Both
  opt out with `--no-agents-md`, both are skipped for the personal vault (its parent is the
  home directory, not a project), and `okf skills install --scope project` writes the
  identical pair.
...
| C# style | File-scoped namespaces everywhere (161 of 161 files under `src/` and `tests/`). Ringo's 2026-08-15 rulings (#31) are build errors from here: private instance fields are `_camelCase` (no `this.`-qualified member reference remains — the prefix disambiguates, so `dotnet_style_qualification_for_field` is `error` too), private `static` fields (readonly or not) stay `PascalCase` — the ruling is silent on statics, so they default to what all 37 already were. `var` is `false:error` in `src/` (explicit types plus target-typed `new`; IDE0008); `[tests/**/*.cs]` reverts it to `true:silent` because the ruling names a rewrite, not a test-code scope. `Nullable`, `TreatWarningsAsErrors`, `AnalysisLevel=latest-all`, `EnforceCodeStyleInBuild` and `CodeAnalysisTreatWarningsAsErrors` are set once in `Directory.Build.props` for all four projects, and the repo-root `.editorconfig` holds the style, naming and every analyzer downgrade, each with its reason (`docs/spikes/2026-08-16-analyzers.md`). |
...
| [#54](https://gitlab.tychostation.dev/ringo/okf-net/-/issues/54) — concurrent registry writes | Registry writes are atomic but a concurrent read-modify-write loses entries. Ringo must choose an advisory lock and retry policy or make serial writers an explicit constraint. |
| [#55](https://gitlab.tychostation.dev/ringo/okf-net/-/issues/55) — dogfood source-pin policy | The bundle's commit-pinned source URLs predate claims added later. Ringo must choose whether the custodian re-pins on every edit, cites the moving branch with `last_modified`, or accepts the stale pins. |
| [#57](https://gitlab.tychostation.dev/ringo/okf-net/-/issues/57) — `okf concept move` | A deterministic move has to settle cross-bundle dangling links and rewrite vault-relative capture-manifest pointers without weakening the existing containment and write rules. |
| [#58](https://gitlab.tychostation.dev/ringo/okf-net/-/issues/58) — `okf bundle add` | Installing a distribution adds verified extraction and, for URLs, a second user-invoked network path. Curated bundles stay outside the binary; whether setup offers one belongs to #50. |
