...
- **`sha256`, and the field is spelled exactly as the capture manifest spells it.** One
  ...
  is the single implementation [corrected 2026-08-15, see below], and `raw/` immutability
  (`OKF0310`) and distribution integrity now answer to the same primitive.
...
in favour of ~250 lines of our own [corrected 2026-08-15, see below]: a Fruchterman-Reingold
layout on a `<canvas>`, plus the dashboard's filtering. The reasoning is not "not invented
here":
...
count [corrected 2026-08-15, see below], nothing reads a clock beyond today's date
(injected, per CORE-7), and page order is ordinal. Two runs over an unchanged vault produce byte-identical output, which a CLI test
...
### Corrections after the 1.0.0 architecture fact-check (2026-08-15)

Work item #28 built `docs/architecture.md` from this log, and fact-checking every `AD` in it
against the code turned up claims in **this file** that the code disagrees with. They are
corrected here rather than edited away at the point they were written: each was a recorded
decision, and a decision log that silently agrees with the present is a log nobody can audit.
Each wrong sentence above now carries `[corrected 2026-08-15, see below]` and is otherwise
untouched.

Nothing below changes a decision. Every one of these is a wrong *number* or a wrong
*description of the mechanism*; the call each passage was making — no third-party
JavaScript, deterministic generation, one hash convention — still stands, which is exactly
why the wrong detail survived review for as long as it did.

- **The graph is 393 lines, not "~250".** Measured on `src/Okf.Core/Assets/site.js`, whose
  section-2 block (the `2. Force-directed graph, on a canvas.` banner through the close of
  `initGraph`) runs lines 298–690 = **393**; the whole file is 754 lines, the rest being the
  dashboard's filtering and the single-file router. `AD-39` says "about 390 lines", which is
  the honest rounding. The estimate was written before the code existed and never revisited;
  it understated the work by about 60%, which strengthens rather than weakens the argument
  against vendoring `cytoscape.min.js` at ~400 KB — 393 lines of reviewable source is still
  three orders of magnitude less to take on faith. The companion sentence about Markdig ("a
  CommonMark implementation is not 250 lines") is reasoning against the same original
  estimate and stays as written; read it as "not a few hundred lines", which is still true.
- **Site generation is not "a seeded PRNG and a fixed iteration count" — because the
  generator does not lay the graph out at all.** The generator emits **no layout coordinate**.
  Every node position is computed in the reader's browser on load, so the determinism claim
  the passage was making is true for a different reason than it gives: the generated bytes
  are identical across runs because they contain no positions to differ, not because a
  seeded layout produced the same ones twice. In the browser, the seed is a client-side LCG
  (`seed = (seed * 1103515245 + 12345) % 2147483648`, seeded at `1`) and the iteration count
  is **not fixed** — it is `n > 300 ? 160 : 400`, adaptive to node count, under a cooling
  schedule. `AD-40` states it correctly: the layout "is laid out in the reader's browser from
  a seeded PRNG under an iteration bound, so it is reproducible per reader rather than per
  build". Two readers on the same site see the same graph; the site itself never recorded one.
- **`OkfCaptureManifest.Sha256Of` is not "the single implementation" — there are two forms of
  one convention.** `Sha256Of(string path)` opens a file and hashes it, and every file on
  disk goes through it. The archive verifier holds a stream and no path, so it uses the
  one-line stream form beside it, `OkfBundler.Digest(Stream)`. Both are
  `Convert.ToHexStringLower(SHA256.HashData(…))` and both produce the same 64 lowercase hex
  digits, so the *convention* is genuinely single and the digests are genuinely comparable —
...
- **The bundle walk's ordering claim was already corrected in place, in the multi-platform
  ...
  `okf index --json`'s entry order". Neither was true: `okf index --json` is the index plan's
  own re-sort, and `okf lint` is `OkfDiagnostic.CompareTo` applied as a total re-sort that
  discards the walk order — fixed separately in commit `481544b`, *fix(core): order lint
  diagnostics by the spec's separator, not the host's*. What the
  walk does decide is `okf inbox` and which of two near-duplicates keeps `OKF0303`. That
  correction landed in the section itself (commit `acb2de8`, 2026-08-15) because the wrong
  sentence was a *reason* attached to a change being described, not a decision anybody had
  ratified. It is listed here only so this section is a complete index of what the
  fact-check found.
- **`OKF0203` names no rule, and the stragglers are gone.** Source drift is `OKF0103` in the
  provenance range; the trust range ships `OKF0201` and `OKF0202` only. Prose and comment
  references named the missing id while describing exactly what `OKF0103` does, and they
  were corrected in commit `bd15372`: ten occurrences across six files — two here, one in
  `okf/bundles/okf-net/practices/custodian-model.md`, and seven in source and tests. (That
  commit's message says nine; the diff carries ten.) Verified
...
(three binaries, the knowledge archive, `latest.json`, and both installers), and
...
otherwise be re-learned the same way it was just learned. Over-capture is the risk this
...
guide's terms: it names out-of-context material (the three skills, `okf search`, `okf
inbox`) and encodes the condition for reaching each — one branch per skill, front-loaded
with a **leading word** (*Read*, *Capture*, *Maintain*) so each line anchors a single
behavior rather than spelling it out. Every clause is phrased positively (no "don't"), the
block restates nothing `okf help` or a skill's own `description` already says — duplicating
either would inflate this always-loaded fragment's cost for a fact the agent can already
reach — and the read branch's escape hatch (`okf search` / `okf inbox` directly) is the one
genuinely needed: it covers the skill not being installed at all, a case the skill's own
text cannot cover for itself. No second escape hatch for `okf` missing from PATH is added,
because `okf-vault`'s own body already carries one ("With `okf` absent from the PATH and no
documented equivalent in the project, say so and stop") — restating it here would be exactly
the duplication the guide's pruning section warns against. A copy of both guidance files —
`SKILL.md` and `SKILL-MECHANICS.md` — is kept at `docs/reference/writing-for-agents/` so
this citation resolves inside the repository rather than at a path only the author's machine
has (Ringo's skill, 2026-08-16, copied as of this decision).
...
`okf init`; this block is six lines whose exact bytes are the fence contract itself, and
