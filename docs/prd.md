...
| `okf skills <list\|path\|install>` | Report the agent skills this binary carries, where one is installed, or install them | `--host`, `--dir`, `--scope <user\|project>` (an install target, not a search scope), `--force`, `--names` (`list` only — bare names for a script), `--no-agents-md` (with `--scope project`) | 0 installed or reported · 1 `path` for a skill that is not installed · 2 usage |
| `okf completion <bash\|zsh\|fish\|pwsh>` | Print the completion script for one shell, generated from okf's own verb table | none | 0 script on stdout · 2 no shell, an unknown shell, or usage |
