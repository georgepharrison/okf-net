...
internal static class CompletionCommand
{
    ...
    private static string Bash()
    {
        ...
        {
            Line($"        {verb.Name})");
            var groups = ValueGroups(verb).ToList();
            if (groups.Count > 0)
            {
                Line("            case \"$prev\" in");
                foreach (var (value, flags) in groups)
                {
                    var pattern = string.Join("|", flags);
            ...
            var guard = OperandCondition(verb, "$sub");
            var indent = guard.Length > 0 ? "                " : "            ";
            if (guard.Length > 0)
            {
                Line($"            if [ {guard} ]; then");
            }
    ...
    private static string Fish()
    {
        ...
        foreach (var verb in CompletionTable.Verbs)
        {
            Line("");
            Line($"# okf {verb.Name}");
            var condition = $"__fish_seen_subcommand_from {verb.Name}";

            foreach (var flag in verb.Flags)
            {
                var spelling = flag.Name.StartsWith("--", StringComparison.Ordinal)
                    ? $"-l {flag.Name[2..]}"
                    : $"-s {flag.Name[1..]}";
                var value = flag.Value.Kind switch
                {
                    "none" => string.Empty,
                    "path" => " -r -F",
            ...
            var seen = verb.OperandSubcommands.Count > 0
    ...
    private static string PowerShell()
    {
        ...
        foreach (var verb in CompletionTable.Verbs)
        {
            ...
            foreach (var (value, flags) in ValueGroups(verb))
            {
                var words = value.Kind switch
                {
                    "path" => "@()",
            ...
            {
                var operands = string.Join(", ", verb.Operand.Candidates.Select(candidate => $"'{candidate}'"));
        ...
        var verbNames = string.Join(", ", CompletionTable.Names.Select(name => $"'{name}'"));
